...
class Elf:
	...
	def extract_section_headers(self):
		...
		for s in self.section_headers:
			s.name = self.section_names[s.name_offset:self.section_names.find(b'\x00', s.name_offset)].decode('utf-8')
		return self.section_headers
	...
	def generate_updated_elf(self):
		...
		for name, body in self.new_sections:
			try:
				...
				# Ensure name is bytes
				if isinstance(name, str):
					name_bytes = name.encode('utf-8')
				else:
					name_bytes = name
				self.section_names += name_bytes + b'\x00'
				s = ElfSectionHeader([name_off, 1, 0, 0, -1, -1, 0, 0, 1, 0], file_offset=-1)
				s.name = name if isinstance(name, str) else name.decode('utf-8')
...
class DumbEnum(object, metaclass=DumbEnumMeta):
	pass
